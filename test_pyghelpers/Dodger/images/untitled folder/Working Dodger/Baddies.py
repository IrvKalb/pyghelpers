...
class BaddieMgr():
    ...
    def reset(self):  # Called when starting a new game
        self.baddiesList = []
        self.baddieCounter = 0  # add a new baddie every ADD_NEW_BADDIE_RATE frames

    def update(self):
        # If the correct amount of frames have passed,
        # add a new baddie
        self.baddieCounter = self.baddieCounter + 1
        if self.baddieCounter == BaddieMgr.ADD_NEW_BADDIE_RATE:
            # Time to add a new baddie (and reset the counter)
            oBaddie = Baddie(self.window)
            self.baddiesList.append(oBaddie)
            self.baddieCounter = 0
